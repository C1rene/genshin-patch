...
export default class MysSign extends base {
    ...
    async signWithImage(cks, uids) {
        ...
        for (let g of this.set.game) {
            ...
            for (let i = 0; i < uids[g].length; i++) {
                ...
                let verificationUser = VerificationStats.context(this.e, this.e?.user_id)
                let res = await this.doSign(ck, uid, g, name, verificationUser)
                if (res.retcode === -1000)
        ...
        screenData.tplFile = `${this._path}/plugins/genshin/resources/MysSign/html/sign/sign.html`
        screenData.pluResPath = `${this._path}/plugins/genshin/resources/MysSign/`
        let data = {
            ...screenData,
    ...
    async doSign(ck, uid, game, name, verificationUser = {}) {
        ...
        let res = await this.bbsSign(name, verificationUser)
    ...
    async bbsSign(name, verificationUser = {}) {
        this.signApi = true
    ...
    async signTask(manual, targetQQs = null) {
        ...
        if (busyQQs.size > 0) {
            for (let game of this.set.game) {
                uids[game] = uids[game].filter(uid => !busyQQs.has(String(cks[game]?.[uid]?.qq ?? '')))
                cks[game] = Object.fromEntries(Object.entries(cks[game]).filter(([, ck]) => !busyQQs.has(String(ck?.qq ?? ''))))
            }
            logger.mark(`[签到队列]跳过正在执行前置签到的QQ：${[...busyQQs].join(',')}`)
        }
        ...
        if (Array.isArray(targetQQs))
            tips.push(`\n按当前主人命令范围签到，共${targetQQs.length}个QQ`)
        else
            tips.push('\n按当前自动签到权限范围执行')
        ...
        for (let g of this.set.game) {
            ...
            for (let i = 0; i < uids[g].length; i++) {
                ...
                logger.mark(`自动签到[${name}]用户·QQ:${ck.qq} UID:${ck.uid}`)

                let retry = 0
                let verificationUser = manual
                    ? VerificationStats.context(this.e, this.e?.user_id)
                    : VerificationStats.context(null, ck.qq)

                let ret = await this.doSign(ck, ck.uid, g, name, verificationUser)
                if (ret.retcode === -1000)
                    while (ret.retcode === -1000 && retry < this.set.retry) {
                        ret = await this.doSign(ck, ck.uid, g, name, verificationUser)
                        retry++
                    }
